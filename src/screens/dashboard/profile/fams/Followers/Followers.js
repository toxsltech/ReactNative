...
class Followers extends React.Component {
  ...
  afterFollowers = () => {
    const data = this.props.getfollowersResponse ? this.props.getfollowersResponse.data : '';
  ...
  fetch = async () => {
    ...
    fetch(BASE_URL + 'auth/getAllUserProfile/' + postID, {
      ...
      .then((json) => {
        ...
        const followOtherData = OtherData[0].map((item) => item.followBy)
  ...
  getData = async () => {
    try {
      ...
      if (refreshvalueotherprofile == 'true') {
        ...
        this.props.Followers().then(() => this.afterFollowers())
      }
    } catch (e) {
  ...
  render() {
    const { followersData, followOtherData, userId, otherUserId, refreshvalueotherprofile } = this.state;
          ...
          renderItem={({ item, index }) => {
            return (
              <FollowersCard
                ...
                status={item.status}
                following={refreshvalueotherprofile ? item.isfollowedByUser : true}
