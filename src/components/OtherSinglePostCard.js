...
class OtherSinglePostCard extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isOptionOpen: false,
    ...
    fetch = async () => {
        ...
        fetch(BASE_URL + 'auth/getAllUserProfile/' + postID, {
            ...
            .then((json) => {
                ...
                const postData = Info1.map((item) => item.postData)
                const comments1 = postData[0].map((item) => item.countComment)
                const likes1 = postData[0].map((item) => item.countLike)
                const isLikedByUser1 = postData[0].map((item) => item.isLikedByUser)
                const viewCount1 = postData[0].map((item) => item.viewCount)
                const viewedBy2 = postData[0].map((item) => item.viewedBy)
...
export default OtherSinglePostCard;
