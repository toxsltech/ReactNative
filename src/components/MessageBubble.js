...
class MessageBubble extends React.PureComponent {
  ...
  render() {
    const { sent_by, profile_image, type, text, file, seen, index, length, image_url, thumbnail } = this.props
          ...
          <View
            ...
            <View
              ...
              <TouchableOpacity onPress={() => this.setState({ isOpen: true })}>
                <Image source={{ uri: SOCKET_IO_SERVER + image_url }} style={styles.postImage} />
              </TouchableOpacity>
              {text.length > 0 && (
