...
import { View, Text, Image, TouchableOpacity, Modal, Share, Dimensions, ActivityIndicator } from 'react-native';
...
class PostCardHome extends PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      isOptionOpen: false,
  ...
  async componentDidMount() {
    try {
      ...
      this.setState({ loginuserid: USERID })
    } catch (e) {
  ...
  HASHTAG_FORMATTER = string => {
    return string.join(" ").split(/((?:^|\s)(?:#[a-z\d-]+))/gi).filter(Boolean).map((v, i) => {
      if (v.includes('#')) {
        return <TouchableOpacity onPress={() => this.hashtag(v)}>
          <Text key={i}
            style={{
              color: '#0089FF',
  ...
  render() {
    const { isOptionOpen, modalVisible } = this.state;
    const { profile_image, user_name, likes, comments, description, post_type, post_url, isLikedByUser, views, isLive, thumbnail, postID, userId, following, viewedBy } = this.props;
    ...
    return (
      <View style={styles.container}>
        {isOptionOpen && (
          <View style={styles.options}>
            <TouchableOpacity style={styles.optionItem} onPress={() => {
              !following ? this.onFollowRequest(userId) : this.onUnFollowRequest(userId)
            }}>
              <ResponsiveText style={styles.optionText}>{following ? 'Unfollow' : 'Follow'}</ResponsiveText>
            </TouchableOpacity>
        ...
        <View style={styles.postHeader}>
          <TouchableOpacity
            onPress={() => this.profilepage(userId)} >
          ...
          <View style={styles.descriptionTextContainer}>
            {description ?
              <Text style={styles.descriptionText}>
                ...
                {/* {this.HASHTAG_FORMATTER(description)} */}
              </Text>
              : null}
            <TouchableOpacity
